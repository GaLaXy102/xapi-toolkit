    ...
    private void seed() {
        try {
            if (this.daveQueryRepository.count() == 0) {
                DaveQuery query = new DaveQuery("Learners Engagement",
                        "[:find (count ?s) ?c\n" +
                                "  :where\n" +
                                "  [?s :statement/actor ?a]\n" +
                                "  [?a :agent/mbox ?c]]");
                this.daveQueryRepository.save(query);
            }
            if (this.daveGraphDescriptionRepository.count() == 0) {
                DaveGraphDescription graph = new DaveGraphDescription("Top 10 DESC",
                        "{\n" +
                                ...
                                "  ]\n" +
                                "}");
                this.daveGraphDescriptionRepository.save(graph);
            }
            if (this.daveVisRepository.count() == 0) {
                DaveVis vis = this.daveVisRepository.save( new DaveVis("Learners Engagement",
                        this.daveQueryRepository.findByName("Learners Engagement").get(),
                        this.daveGraphDescriptionRepository.findByName("Top 10 DESC").get(), true));
